...
THIS VERSION MODIFIED BY: Arsalan Adil; May 2020
Added functionality: 
-->Plot w_scf
--> Plot a instead of z on x-axis
--> Ability to oo smeilogy (log-y lin-x axes)
...
def CPU_parser():
    ...
    parser.add_argument('--scale', type=str,
                        choices=['lin', 'loglog', 'loglin', 'semilogy','george'],
...
def plot_CLASS_output(files, x_axis, y_axis, ratio=False, printing='',
    ...
    if not ratio:
        for index, curve in enumerate(data):
            ...
            if 'w_scf' in y_axis:
                p_scf_ref = tex_names.index("(8\\pi G/3)p_scf")
                rho_scf_ref = tex_names.index("(8\\pi G/3)rho_scf")
                eqnofstate = plot_w_scf(data_arr[:,p_scf_ref],data_arr[:,rho_scf_ref])
                ax.semilogx(1/(1.+data_arr[:,0]), eqnofstate)
            ...
            x_index = 0
            x_to_plot = 1/(1. + data_arr[:,0])#AA:Make default x-axis to "a" instead of "z"
            if x_axis:
                for index_name, name in enumerate(names):
                    if name.find(x_axis) != -1:
                        x_index = index_name
                        break
                x_to_plot = data_arr[:,x_index]
            ...
            for selec in y_axis:
                #
                #replace all ax.plot below with
                #ax.plot(x_to_plot, unchanged)
                #
...
def plot_w_scf(p_scf_data,rho_scf_data):
    eqnofstate = p_scf_data/rho_scf_data
    return eqnofstate
